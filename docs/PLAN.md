...
| ID | 任务 | 依赖 | 档位 | 状态 |
|---|---|---|---|---|
| T-021 | 启用并真实验证 rerank（契约核对 → 对齐 → A/B → 降级验证） | — | L1 / T1 | **待办** |
...
改动文件：<按实际填>
验收结果：
  [ ] compileall 退出码 0 —— 实际：<粘贴>
  [ ] 离线单测全绿（Ran N tests, OK）—— 实际：<粘贴>
  [ ] MOCK=1 不发请求的离线单测 —— 实际：<用例名与输出>
  [ ] Step 1 连通性：走了 A / B 路径，原始响应见 E-04 §x —— 实际：<粘贴关键字段>
  [ ] Step 2 A/B 对照表（含是否降级）—— 实际：<粘贴>
  [ ] Step 3 降级验证：[warn] rerank 降级 出现且未抛异常 —— 实际：<粘贴>
  [ ] RERANK_ENABLED 默认仍为 0 —— 是
遗留问题：<无 / 具体描述>
下一步建议：<是否建议改默认值 / 换 qwen3-rerank>
```

...
> ✅ **结项补充（2026-09-21，已执行完）** —— **M9 是本项目第一个正面结果**，也是这条证据链的收口：
...
14. **小节级整理（`SECTION_MODE`）实测无效，默认保持 `off`（T-020，2026-09-21）**：新增的环境变量 `SECTION_MODE` 有三档 —— `off`（默认，行为与改动前逐字一致）/ `diverse`（同一小节只留最高分块）/ `expand`（命中块所在小节整节进候选）。
     ...
     - **遗留矛头（已由 T-021 接手并验证）**：`src/retrieve.py` 的 rerank 路径当时**已实现但 `RERANK_ENABLED=0` 从未启用、也从未用真实 API 验证过**。
       E-01/E-02/E-03 共同指向**排序**而非召回 —— **T-021 已完成该验证（结论见 §5.15 与 `EXPERIMENTS.md` E-04：有效，+0.088，代价 +2.0 s）**

15. **rerank 实测有效，但默认仍关闭；代价是每题约 +2 s（T-021，2026-09-21）**：同进程 A/B（唯一自变量 `use_rerank`，见 `EXPERIMENTS.md` **E-04**）
    ...
    - **怎么引用**：说「同进程 A/B、单次运行、逐题升 4 降 1、主指标 +0.088、延迟 +2.0 s」，并**主动说出 `q14` 变差**（0.500 → 0.250）。
      **不要**说"提升了 8.8 个百分点"（n=20、单次运行，不是统计检验），**更不要**说"解决了多跳问题"
    - **静默降级是这里最大的运维陷阱**：`_rerank` 的任何异常都被 `search()` 吞掉、只 `print("[warn] rerank 降级: ...")`。
      好处是服务不会挂（已实测：错 key → 401 → 降级 → 结果与融合结果逐字一致）；
      **代价是"rerank 没效果"这种结论可能只是降级了**。⇒ 任何基于 rerank 的对比实验，**必须先确认没发生降级**（E-04 用调用计数 + 顺序改变 + 候选集不变式三条证据）
    ...
    - **未做的事（都属新卡）**：`RERANK_ENABLED` 默认值、`TOP_K_FINAL` 4→8、rerank 与 `top_k` 的联合调参、

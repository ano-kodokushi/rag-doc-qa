...
✅ **判据预注册（本次提交不含任何执行结果）**：见下方任务卡 `T-028`。
...
22. **`max_chars=3000` 是契约里的默认值，且它只在"证据已在窗口内"时才可能成为瓶颈（T-028 开卡，2026-09-21，0 元）**：
    ...
    - **它只对一部分失败有效**：E-07 的逐 keypoint 表显示 `q13` 的要点证据在 `k=4/8/12/15` **恒为 0/1**、
      `q16` 的「运行时编译」四个 `k` **恒为 0/1** ⇒ 这类 keypoint **不是被截断的，是根本没进窗口**，`max_chars` 再大也救不了
    - ⚠️ **因此"放大 `max_chars` 就能治多跳"目前没有证据支持**；T-028 的 Step 1 就是去数
      "到底有几个多跳 keypoint 属于**被截断**那一类" —— 若 ≤1 个，该方向应在**花钱前**就终止

...
| ID | 任务 | 依赖 | 档位 | 状态 |
|---|---|---|---|---|
| T-024 | `k=12/15` 的 LLM 侧验证（**先 0 元测 prompt 预算**，再决定是否花约 0.6 元） | T-023 | L1 / T1 | **待办** |
...
17. **小节级覆盖的量化：H2 成立，多跳有四个病因，且"扩大窗口"远没到头（T-023，2026-09-21，0 元）**：
    ...
    **多跳（5 题）**：文件级 **4/5** → **小节级 2/5** → 块级 **1/5**（`k=8, off`）；`k=4, off` 为 3/5 · 2/5 · 1/5。
    ...
    - ⚠️ **我自己推翻了一条判断**：原写"扩大窗口这条线到头了"，**错**。把 `k` 扫全：多跳小节级 `k=4` 2/5 → `k=8` 2/5 → **`k=12` 3/5 → `k=15` 4/5**；块级 `k=8` 1/5 → **`k=12` 3/5**。
      **需要的小节常排在第 9~15 名**，而 E-02/E-05 只比较了 4 与 8，**正好卡在拐点之前** ⇒ **`top_k` 回到第一候选**（必须同时报 prompt 长度与截断题数，E-03 的教训）

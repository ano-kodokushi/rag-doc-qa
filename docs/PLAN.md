...
| ID | 任务 | 依赖 | 档位 | 状态 |
|---|---|---|---|---|
| T-016 | multi-hop 检索侧改进的对照实验 | — | L1 / T1 | **完成**（负结果） |

> **M6 闭环，结论是负的**：把 `TOP_K_FINAL` 从 4 扩到 8，**`multi_hop` 一分未动（仍 1/5）**，
> 而融合池里两篇依据本来就 **5/5 全覆盖** → **多跳题的瓶颈不在检索召回，在生成侧的跨文档综合**。
> 完整设计、原始数据与方法学限制见 `docs/EXPERIMENTS.md` 的 **E-01**。
...
6. **`multi_hop` 是当前最弱项，而且瓶颈位置已经被实验定位（T-016 / E-01）**：分类实测 `multi_hop` accuracy **0.20**（5 题：1 完全对 / 3 部分对 / 1 错）、`single_hop` **0.50**（12 题：6/2/4）、`unanswerable` **1.00**（3 题全部正确拒答）。
   **原先写的「疑似原因：跨文档的另一半依据被 top-4 挤掉」已被实测推翻一半**（详见 `docs/EXPERIMENTS.md` E-01）：
   - 融合池里两篇依据 **5/5 全覆盖**（召回没问题）；top-4 截断只影响 5 题里的 2 题
   - **把 `TOP_K_FINAL` 从 4 扩到 8，`multi_hop` 一分未动**；而 `q14`/`q16`/`q17` 两篇依据**都在 top-4 里**却只拿到"部分对"
   - → **瓶颈在生成侧的跨文档综合，不在检索召回**。所以"扩大召回"这条路对多跳题无效
   - 若将来要做父子分块，应把它设计成**生成侧改进**（给综合提供更完整的上下文），而不是检索侧改进
   - 多跳题的 `keypoints` 天然更多、更容易落在"部分对"，这是评分侧的另一层原因
7. **`hit_at_k` 对 k 单调，因此 `hit_rate` 不能跨 k 比较**（T-016 副产品发现）：k 越大，`gold_sources` 被覆盖的机会越多，命中率会**机械上升**。
   所以 V2（`top_k=8`）的 `hit_rate 0.80 → 0.85` **不是质量提升的证据**；跨 k 比较质量必须用「**全部**依据是否都进 top-k」这类集合级指标（见 `docs/EXPERIMENTS.md`）。

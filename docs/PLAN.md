...
> 最值得先测的下一个假设当时写的是 **`top_k` 扩大 + rerank 联合** —— **该假设即 T-022，已测并被证伪**（见 §2.12 / §5.16 与 `docs/EXPERIMENTS.md` E-05）；
> 目前证据指示的下一个方向是 **先量化「小节级」覆盖**（0 元测量），详见 §5.16 末条。
>
...
## 2.12 里程碑 M10 · `top_k_final` × rerank 联合（把「多跳为什么没修好」再逼一步）

| ID | 任务 | 依赖 | 档位 | 状态 |
|---|---|---|---|---|
| T-022 | `top_k_final` × rerank 联合实验（先 0 元机制诊断，再决定是否花 0.9 元） | T-021 | L1 / T1 | **完成**（**H1 被证伪**；Step 2 未执行，省下 0.9 元） |

> **假设从哪来（E-02 与 E-04 的交叉点）**
> - E-02：`top_k` 4→8 → 多跳覆盖率 **0.450 → 0.550（5 升 0 降）** —— **扩大窗口对多跳有效**
> - E-04：rerank 开着但窗口仍 `top_k_final=4` → 多跳覆盖率 **0.500 → 0.500（±0.000）** —— **重排对多跳无效**
>
> **H1（可证伪）**：rerank 现在是在**替多跳题做取舍** —— 它在只有 4 个席位时把"第二跳"那篇挤出去（`q14` 正是这个表现）。
> 如果**先把窗口扩到 8 再重排**，rerank 就只需要排序、不必排除，多跳可能因此受益。
>
> **怎么证伪**：看 (k=8, rerank=on) 的多跳覆盖率**是否高于** (k=8, rerank=off)，且**高于** E-04 的 (k=4, rerank=on) = 0.5000。
> 若三者持平，H1 死，且"多跳瓶颈不在排序、也不只在窗口"得到第二次确认。
>
> **先花 0 元的理由（跟 T-016/T-021 的 Step 0 同一个套路）**：机制诊断只看**检索窗口里有没有那篇依据**，
> 不需要任何 LLM 调用。**如果 k=8 连"第二篇依据都进不了窗口"，那 H1 在花钱之前就已经死了。**
>
> **红线**：不改 `SPEC.md`、不改任何默认值、不改题库判分；**必须分题型报**（E-04 的教训）；假设被证伪就如实写成负结果。
>
> ---
>
> ✅ **结项（2026-09-21）：H1 被证伪，Step 2 未执行 —— 0 元的机制诊断省下了约 0.9 元。**
>
> **Step 1 实测（0 元 LLM，只有 query embedding）**——多跳题「两篇依据都进窗口」：
> `(k=4,off)` **3/5** · `(k=8,off)` **4/5** · `(k=4,on)` **2/5** · **`(k=8,on)` 3/5**
> → H1 预测 `(k=8,on) ≥ (k=8,off)`，实测**低于**。**席位从 4 扩到 8，rerank 仍然挤掉依据。**
>
> **最干净的反例是 `q15`**：`k=8` 且**不开** rerank 时三篇依据全进窗口；**一开 rerank 就掉一篇** ——
> **席位足够也没用 ⇒ 挤依据不是"位置不够"，而是 rerank 的目标函数（单 query 相关性）与"跨文档互补覆盖"本来就不一致。**
> 唯一符合 H1 的是 `q14`（扩窗口确实救回来了）—— 所以 **H1 不是全错，而是"只在个别题上成立"**。
> 另有 `q13` 四种配置全缺同一篇 ⇒ 这类题与窗口/重排**都无关**，问题在融合池排名。
>
> **顺带得到的解释（与 E-04 自洽）**：rerank **从不通过"增加依据覆盖"生效** ——
> `k=4` 开它前后平均命中依据数都是 1.000，可 `accuracy` 却升了；`k=8` 开它反而让覆盖变差（1.100 → 1.050）。
> ⇒ **它的贡献是"把已有的块排到更前面"，不是"把缺的块找回来"** —— 所以单跳受益、多跳受害。
>
> **未测的边界（必须一起说）**：本卡**没有** LLM 侧数据，所以「`(k=8,on)` 的**答案**是否好于 `(k=4,on)`」仍未验证。
> **"机制失败"不等于"答案一定更差"**，不能说成"已证明 k=8 + rerank 没有价值"。详见 `EXPERIMENTS.md` **E-05 §6**。
>
> **下一步该量的是「小节级」覆盖**：`k=8,off` 下多跳题已经 **4/5 文件级齐全**，答案却仍然差 ——
> "文件在窗口里"≠"需要的那一小节在窗口里"（E-02 的结论）。这是**比再花 0.9 元更该先做的 0 元测量**。

## 2.13 里程碑 M11 · 小节级覆盖的量化（多跳题缺的到底是"文件"还是"小节"）

| ID | 任务 | 依赖 | 档位 | 状态 |
|---|---|---|---|---|
| T-023 | 小节级覆盖的量化（0 元；把"文件在窗口里"拆到"小节 / 块"两级） | T-022 | L1 / T1 | **完成**（**H2 成立**，且拆出多跳的四种病因） |

> **假设从哪来**
> - E-02：多跳的瓶颈是「检索到同一份文件的**错误小节**」（但当时只有**定性**证据：`q03`/`q12`/`q14`/`q16`/`q17` 缺的内容"全在语料里"）
> - E-05：`k=8` 且不开 rerank 时，多跳题已经 **4/5 文件级齐全**，答案却仍然差
>
> **H2（可证伪）**：把口径从「文件」降到「小节 / 块」之后，覆盖率会**明显低于**文件级 ——
> 即模型眼前的窗口里**有那份文件、但没有那一节**。
>
> **怎么证伪**
> - 若「小节级覆盖率」≈「文件级覆盖率」⇒ **H2 死**，E-02 的定性结论站不住，多跳的锅不在小节粒度
> - 若明显更低 ⇒ H2 成立，**并能量化差多少**（这正是 E-02 缺的那个数）
>
> **为什么必须先做这一步（0 元）**：它决定下一步往哪走 ——
> H2 成立 ⇒ 该做"小节感知的检索 / 召回"；H2 不成立 ⇒ 责任回到生成侧（那 E-01 的第一版结论反而有一半是对的）。
> **在花任何钱之前，这个问题必须先用现成数据回答。**
>
> **判据（预注册，写完就不再改）**
> 1. 主指标：**多跳题（5 题）「证据所在小节全部进窗口」的题数**，与**同一配置的文件级**口径对比
> 2. 次指标：**块级**（证据 chunk 是否在窗口）与**融合池级**（证据是否在池子里）—— 用来定位"丢在哪一步"
> 3. **对照配置**：`(k=4,off)` 与 `(k=8,off)`（**不含 rerank**：E-04/E-05 已证 rerank 对多跳是净负，先看干净基线）
> 4. **"证据"的定义必须先写死并报告可定位率**：用题目的 `keypoints` 去 `chunks.jsonl` 做**归一化子串匹配**，
>    命中的 chunk 即该题的证据 chunk；**定位不到的 keypoint 必须单独报数**，不得混进分母
> 5. 判据的**调整（若有）必须写进 E-06 并说明原因**，不得事后改上面这四条
>
> ---
>
> ✅ **结项（2026-09-21）：H2 成立，并拆出多跳的四种病因。判据 5 条无调整。**
> **判据先于结果有 git 证据**：判据见本节的提交 **`5c33943`**（"开卡…判据先于结果，单独提交"），结论是其后另一次提交 —— `git show 5c33943` 即可核验（T-022 流程教训的直接兑现）。
>
> **实测（0 元 LLM 费用）**——多跳 5 题的覆盖率分解：
>
> | 来源 | 文件级 | **小节级** | 块级 |
> |---|---|---|---|
> | 融合池 | **5/5** | 4/5 | 3/5 |
...
>
> ⇒ **文件级 4/5、小节级只有 2/5：窗口里有那份文件、但没有那一节** —— E-02 的定性结论**第一次被量化**。
> **小节级未过的是 4 题**（`q13`/`q14`/`q16`/`q17`）；其中因「**整个节没进窗口**」(②) 失分的是 **3 题**（`q13`/`q14`/`q17`），`q16` 属③（节在、承载要点的块不在）。
>
> ⚠️ **我在结项后自行推翻了一条判断（独立验证的线索触发，0 元补测）**：
> 我原写"扩大窗口这条线已经走到头"（依据 `k=4→8` 多跳小节级 2/5→2/5）。**把 `k` 扫全之后这是错的** ——
>
...
> | 块级 | 1/5 | 1/5 | **3/5** | 3/5 |
>
> ⇒ **需要的小节常排在融合池第 9~15 名**，而 E-02/E-05 只比较了 4 与 8，**正好卡在拐点之前**。
> 所以 **`top_k` 重新成为第一候选**（但必须同时报 prompt 长度与截断题数 —— E-03 的截断教训）。
>
> **最有价值的副产品**：多跳 20 个 keypoint 按"丢在哪一级"归类 ⇒ **四种病因**：
> **① 池子里就没有 3/20**（**召回层**！）· **② 进了池子但节没进窗口 4/20**（排序/截断）·
> **③ 小节在、块不在 2/20**（窗口内块选择）· **④ 全都在场 11/20**（若仍答错 ⇒ 生成）。
> ⇒ 过去四张卡一直在 **②** 这一格打转，而它只占 **4/20**；**① 召回层第一次被量化成 3/20**；
> **E-01 的第一版结论被部分平反**（`q15` 四个要点全在场且答对 1.000；`q16` 3/4 在场却只拿 0.500 ⇒ 在场之后仍答不好的情况确实存在）。
>
> **证据可定位率 53/53（100%）**，但**通用 keypoint 会放宽小节级判定**（`q16` 的 `.vue` 命中 52 个小节）——
> 该偏差**对 H2 不利**，故 H2 成立不依赖它。详见 `EXPERIMENTS.md` **E-06** 与 `PLAN.md §5.17`。

## 2.14 里程碑 M12 · 加大窗口的 LLM 侧验证（覆盖率涨了，答案会不会涨？）

| ID | 任务 | 依赖 | 档位 | 状态 |
|---|---|---|---|---|
| T-024 | `k=12/15` 的 LLM 侧验证（**先 0 元测 prompt 预算**，再决定是否花约 0.6 元） | T-023 | L1 / T1 | **完成**（**H3 在花钱前被证伪**；Step 2 未执行，省下 0.6 元） |

> **假设从哪来**
> - E-06 §3.5（0 元）：**多跳小节级覆盖率随 `k` 单调上升** —— `k=4` 2/5 · `k=8` 2/5 · **`k=12` 3/5** · **`k=15` 4/5**；块级从 `k=12` 起 **1/5 → 3/5**
> - 但 **覆盖率 ≠ 答案**：E-02 已经教过一次（覆盖率 5 升 0 降，`accuracy` 仍是 0.50）
>
> **H3（可证伪）**：把窗口从 4 加大到 12/15，**多跳的小节级/块级覆盖提升会传导到答案**。
>
> **已知的、必须同时测的反向力量（E-03 的教训）**：窗口越大 → prompt 越长 → 越容易撞上 `SPEC.md §5` 的 `max_chars=3000` 硬预算；
> **E-03 的 `expand` 就是这样翻车的**（14/20 题丢资料、覆盖率涨 0.010 但 `q04` 从 1.00 掉到 0.00）。
> ⇒ **本卡必须把 prompt 长度与"被丢资料题数"当成一等公民，与答案指标并列记录**。
>
> **判据（预注册，写完就不再改）**
> 1. **Step 1（0 元）**：给出 `k=4/8/12/15` 的 **prompt 字符中位 / 最大** 与 **被 3000 字符预算丢掉资料的题数**，并逐题列出
> 2. **Step 1 决策点（先写死，再执行）**：
>    - 若 `k=15` 的"丢资料题数 ≤ 4/20" ⇒ Step 2 用 **`k=15`**
>    - 若 `k=15` 丢资料 **> 4/20** 而 `k=12` **≤ 4/20** ⇒ Step 2 用 **`k=12`**
>    - 若 `k=12` 也 **> 4/20** ⇒ **不进入 Step 2**，本卡以"**窗口这条路被 prompt 预算封顶**"的负结果收尾，
>      并把"提高 `max_chars`"登记为新卡候选（改 `SPEC.md` 需用户批准）
>    （阈值 **4/20** 的来源：E-03 里 `expand` 在"4 题最终 prompt 仍超 3000"时已出现 `q04` 崩塌；取 4 作为可接受上限是**先写死的约定**，不是从结果倒推）
> 3. **Step 2（花钱）**：**同进程** A/B —— `(k=4, off)` 与 Step 1 选定的 `k`，各 20 题；**唯一自变量 `top_k`**
> 4. 主指标 **`avg_keypoint_coverage`**，且**必须分题型**报（`single_hop` / `multi_hop` / `unanswerable`）
> 5. **必须同时报**：该次运行的 prompt 长度分布、被丢资料的题数、以及是否出现"覆盖率涨而答案不涨"
> 6. 结论写进 `docs/EXPERIMENTS.md` **E-07**（**正负都写**）；判据若有调整必须写进 E-07
> 7. **不改任何默认值**（`TOP_K_FINAL=4`、`RERANK_ENABLED=0`、`max_chars=3000` 全不动）；**不改 `SPEC.md`**
> 8. `src/**` 预期零改动；若要改必须证明是缺陷而不是调参
>
> ---
>
> ✅ **结项（2026-09-21）：H3 在花钱之前就被证伪 —— 因为 `k≥8` 之后模型看到的内容完全一样。判据 8 条无调整。**
> **判据先于结果有 git 证据**：判据见本节的提交 **`575c3d0`**（"开卡…判据先于结果，单独提交"），结论是其后的另一次提交。
>
> **Step 1（0 元）prompt 预算**：
>
> | `k` | prompt 中位 | prompt 最大 | 被 3000 预算丢掉资料的题数 |
> |---|---|---|---|
> | **4** | **1623.5** | 1936 | **0 / 20** |
> | 8 | **2800** | 2999 | **15 / 20** |
> | 12 | 2800 | 2999 | **20 / 20** |
> | 15 | 2800 | 2999 | **20 / 20** |
>
> **预注册决策点**：`k=15` 与 `k=12` 都丢 **20/20**（> 4/20）⇒ **不进入 Step 2**（省下约 0.6 元）。
>
> **决定性的一级（新增"池 → 窗口 → prompt"第三级口径）**：只有进 prompt 的才是模型看到的。
>
> | `k` | 给的资料 | **prompt 里实际保留（中位）** | **多跳「全部要点的证据块都进 prompt」** |
> |---|---|---|---|
> | 4 | 4 | **4 条** | **1 / 5** |
> | 8 | 8 | **7 条** | **1 / 5** |
> | 12 | 12 | **7 条** | **1 / 5** |
> | 15 | 15 | **7 条** | **1 / 5** |
>
> ⇒ **`k=8/12/15` 送给模型的 prompt 逐字相同**（都被 `max_chars=3000` 截在 7 条资料）。
> **H3 不是"效果小"，是"输入根本没变"** —— 那 0.6 元花下去只会买到一次 LLM 抖动。
>
> **同时推翻 E-06 §3.5 的"结论"部分（数据仍成立）**：那里说"`top_k` 回到第一候选"，
> 实际 **`k≥8` 之后 `top_k` 对模型零影响** ⇒ **窗口不是可用的杠杆，`max_chars` 才是**。
> 这解释了此前几件"看起来矛盾"的事：`k=4→8` 有效是因为 prompt 还在长（1623.5→2800、4→7 条），8 之后被截断；
> rerank 只对单跳有效，是因为多跳需要的"第二篇"常排在第 8 名之后 —— **排到哪儿都没用，模型看不见**。
>
> **唯一剩下的突破口**：**`max_chars`（`SPEC.md §5` 冻结值 3000）**。
> 多跳 5 题里 **4 题**的关键要点在第 4 条资料之后才出现（`q13` 0/4 · `q14` 2/4 · `q16` 2→3/4 · `q17` 2/4），而 prompt 在 3000 字符处硬停。
> ⚠️ **改它要动 `SPEC.md`，必须先取得用户批准**（`§0.2` 明令不得擅改契约）。
> 顺带一份可用的配对依据：`k=4` 时预算有余量、`k=8` 起开始丢资料（15/20）——**将来抬 `max_chars` 应与 `top_k` 同步调**。
>
> **成本**：**约 0.01 元**（只有 query embedding，无任何生成调用）—— 原计划 Step 2 的 0.6 元**未花**。

## 2.15 里程碑 M13 · 三条检索路的对照（把简历里那个空格填上）

| ID | 任务 | 依赖 | 档位 | 状态 |
|---|---|---|---|---|
| T-025 | 仅向量 / 仅 BM25 / 混合（RRF）三路同进程对照 | T-021（同进程 A/B 方法） | L1 / T1 | **完成**（**H4 被证伪：混合反而差 0.093**） |

> **为什么是这张卡**：`RAG项目_准备清单.md` 的 09-22 行要求"加 BM25 混合召回 + Rerank，**用同一套 20 题重跑，产出优化前后对比表**"，
> 并把这句写进简历文案：**"准确率从 ___%（仅向量）提升至 ___%"**。
> **现状：BM25 与混合召回早就实现了，但"仅向量"这条基线从来没跑过** —— `eval/` 里没有任何 vector-only 报告，
> 代码里也没有"只跑一路"的开关。⇒ **简历那个空格至今填不上**，而这正是准备清单**坑 8** 警告过的情况：
> *"只做向量检索就喊'混合检索'…否则对比表做不出提升，简历上'混合检索'就是假的"*。
>
> **H4（可证伪）**：**混合召回（向量 + BM25 → RRF）在这套 20 题上优于仅向量**，且幅度足以写进简历。
>
> **怎么证伪**：若混合与仅向量在 `avg_keypoint_coverage` 上的差异**没有超出运行波动**（`§5.5`：覆盖率 ±0.01~0.02），
> 则 H4 不成立 ⇒ **简历里不得出现"混合检索提升了准确率"这类表述**，只能写"实现了混合召回"（如实降级，不是失败）。
>
> **判据（预注册，写完就不再改）**
> 1. **三格必须同进程**（`§5.12` 已证向量层跨进程会漂移）：`仅向量` / `仅 BM25` / `混合`，各 20 题
> 2. **唯一自变量 = 用哪几路做 RRF 融合**；`top_k_final=4`、`rerank=off`、`SECTION_MODE=off`、`max_chars=3000`、题库与生成模型**全部冻结**
> 3. **实现不得改 `src/**`**：三格都走现成的 `_rankings()` → `_fused_hits()` → 生成，只用 `{"vector": [...]}` / `{"bm25": [...]}` / 两者 三种入参
> 4. **主指标 `avg_keypoint_coverage`**，**必须分题型**报（`single_hop`/`multi_hop`/`unanswerable`）；`accuracy` / `hit_rate` / `avg_latency_ms` 一并报
> 5. **同时报检索侧口径**（0 元可得）：三路的 `gold_sources` 命中率，以及"仅 BM25"能否为每题凑满 `top_k_final` 条
> 6. **不变式检查**：仅向量一格必须与 `_rankings()["vector"][:4]` 顺序一致（证明"只用了向量那一路"，不是别的东西）
> 7. 结论写进 `docs/EXPERIMENTS.md` **E-08**（**正负都写**）；**若 H4 不成立，结论里必须写明"简历那句话要改写"**
> 8. 判据若有调整必须写进 E-08 并说明原因
>
> **成本**：约 **0.9 元**（3 格 × 20 题 = 60 次生成）；Step 1 的检索侧口径 **0 元**。
>
> **红线**
> 1. **不得为了让混合"赢"而挑指标** —— 主指标、分题型、波动区间三条一起报（E-02/E-04 的教训）
> 2. 不得改题库、判分口径、`max_chars`、`SPEC.md` 或任何默认值
> 3. 不得把"仅 BM25 更差"直接当成"混合有效"（**要的是混合 vs 仅向量这一对**）
> 4. H4 不成立时不得含糊收尾 —— **如实降级简历表述，本身也是合格交付**
>
> ---
>
> ✅ **结项（2026-09-21）：H4 被证伪 —— 混合召回反而比仅向量差 0.093。这是本项目第一次否掉自己的核心设计。**
> **判据先于结果有 git 证据**：判据 8 条见本节的提交 **`36543ec`**。执行期间无调整。**`src/**` 零改动**（判据第 3 条）。
>
> **答案侧（三格同进程，唯一自变量 = 用哪几路做 RRF）**：
>
> | 指标 | **仅向量** | 仅 BM25 | **混合（现状）** |
> |---|---|---|---|
...
>
> 逐题（混合 − 仅向量）：**升 2 / 降 4 / 平 14**，主指标差 **低 0.05~0.09**（独立验证复现区间；远超 `§5.5` 的 ±0.01~0.02 噪声带）。
> **⇒ 简历那句"准确率从 ___%（仅向量）提升至 ___%"不仅填不上，方向还相反。**
>
> **分题型揭示了机制 —— 两路各有擅场，混合把弱的那路等权平均了进来**：
>
> | 题型 | **仅向量** | 仅 BM25 | 混合 |
> |---|---|---|---|
...
>
> ⇒ **"两路互补"是真的，但"固定权重 RRF 等权融合"不是好的组合方式。**
>
> **又一次层级教训（第六次），而且这次我自己先写错了一版**：检索侧**四个口径方向不同** ——
> `hit_at_k`（任一命中）**17/16/16** ✅同向 · **"全部 gold 文件进窗口" 13/14/14 ❌反向** · 证据**小节**级 **11/8/9** ✅同向 · 证据**块**级 **7/6/6** ✅同向。
> **只有"文件级全命中"这一个口径是反向的**（它对"进来的是不是需要的那一块"不敏感）。
> ⇒ 正确的机制说法是「**混合让"需要的证据"更难进窗口**」，而不是我初版写的"文件更全、块质量更差"（**方向说反了，已改，见 E-08 §3/§6**）。
> **教训：报检索侧结论必须写清口径 —— 同一份数据能同时支持"混合更全"和"混合更差"**
>
> **简历怎么改写（判据第 7 条要求写明）**：❌ 不能写"混合召回提升了准确率"；
> ✅ 写"实现了两路召回 + RRF 融合，**自建评测后实测混合反而低于仅向量（0.534 vs 0.627），分题型看单跳向量强、多跳 BM25 强**，
> 说明固定权重融合不是最优" ——**敢报反方向结果，比一句没数据的好话强得多**。
>
> **⚠️ 不据此改默认值**：仅向量 0.60 是本项目测到过的最好 `accuracy`，但**单次运行、n=20**；改 `SPEC.md` 的设计需另开卡 + 多轮复跑 + 用户批准。
> **下一个该做的实验是"融合方式"而不是"融合与否"**：两路不同权重 / 按题型路由 / RRF `k` 调参。详见 `EXPERIMENTS.md` **E-08**。

## 2.16 里程碑 M14 · 第二语料域：软件工程实践（`software-engineering`）

| ID | 任务 | 依赖 | 档位 | 状态 |
|---|---|---|---|---|
| T-026 | 第二语料域的语料准备与落地（转换 → `data/raw_se/` → 独立索引） | — | L1 / T2+T1 | **完成**（19 份 / 16.9 万字 / 830 块） |
| T-027 | 新域 20 题题库 + 真实基线评测 | T-026 | L1 / T1 | **完成**（**accuracy 0.90~0.95**；多跳最弱跨域复现） |

> **为什么做第二域（而不是替换或追加）**：用户选定"软件工程实践簇"作为**自有语料**（替代被否决的自有项目文档），
> 并选定**新增第二语料域**，以便**完整保留**现有 12 份语料、20 题、以及 E-01~E-08 的全部实验资产。
> 依据：`eval/README.md:37` —— **题目定稿后不得修改；改题等于换评测集、历史得分不可比**；
> 而现有 20 题的 `gold_sources` 全部指向现有语料 ⇒ 替换或追加都会让历史数字立即失效。
>
> **已核实的可行性（0 元，开卡前已测）**：
> - `RAW_DIR` / `CHROMA_DIR` / `CHUNKS_FILE` / `COLLECTION` **都是可配置项**（`src/config.py:41,46,49,51`），`src/ingest.py` 还有 `--raw-dir`（`:169,185`）⇒ **不改 `src/`、不改 `SPEC.md`** 就能并存第二套索引
> - 本机 **Word COM 16.0 可用**（LibreOffice 也在）⇒ `.doc`/`.docx` 能**正规转换**，不必用"读字节+正则"这种不可靠方式
>
> **语料清单（预注册；来自对 `作业` 文件夹 415 份文件的普查，字数为实际抽取的中文字符数）**
>
> **自有作品 12 份（≈10.6 万字）**：
> `软件项目管理/上交/5组-PBL提交资料/5组-需求规格说明书.doc`(33,686) ·
> `…/5组-投标建议书.doc`(20,858) · `…/第5组-报告-莫浩一-…-(1).doc`(781) ·
> `ERP/PBL/软件工程1班---莫浩一---2023211449.doc`(30,574) ·
> `ERP/实验/软工1班-莫浩一-实验1.doc`(1,126) · `…-实验2.doc`(816) ·
> `未知/2023211449-莫浩一-软件工程1班.docx`(2,555) · `未知/2023211449莫浩一软件认识实习报告.docx`(2,313) ·
> `未知/2023211449莫浩一实习日志.doc`(818) · `系统/第13组-莫浩一-高校教务管理信息系统.doc`(4,547) ·
> `SOA/2023211449-莫浩一-服务科学和SOA阶段报告.doc`(3,260) · `软件体系结构/第3组-莫浩一-软件体系结构PBL.doc`(4,821)
>
> **参考 / 课程文档 8 份（≈6.5 万字）**：
> `软件项目管理/PBL及翻转课堂-投标建议书案例.pdf`(29,115) · `…-需求规格说明书案例.pdf`(15,653) ·
> `软件项目管理/软件项目管理-PBL要求及评分标准-2023级.doc`(5,200) · `ERP/PBL/2 ERP 阶段考试 要求说明.doc`(1,959) ·
> `ERP/实验/5 ERP 实验指导书 2025年 李琰.doc`(1,281) · `ERP/实验/ERP 实验大纲 2025年 2022级李琰.doc`(1,147) ·
> `未知/软件工程-数据库系统原理实验指导书-2025.doc`(9,940) · `SOA/实验指导书-服务科学与SOA基础.doc`(1,188)
>
> **★ 合计 20 份 ≈ 17.1 万字。**
> **明确排除**：`投标建议书案例.docx` 与 `需求规格说明书案例.docx`（与同名 PDF **内容重复**，只留 PDF）·
> `未知/第3组-…软件体系结构PBL.pptx`（与 `软件体系结构/第3组-…PBL.doc` 同一项目，近重复）·
> `kntSoftERP_Free/` 下的说明 txt 与损坏的 `~$P_HELP.doc` · 各 `参考文献.txt`（引文列表，不是正文）·
> `sql/` 与 `数字图像处理/` 的 `.doc`（**抽取出现乱码交错，且不在本簇内**）· 各 `<500 字` 的碎片
>
> ---
>
> ✅ **结项（2026-09-21）：T-026 + T-027 完成。**
> - **T-026**：`data/raw_se/` **19 份**（11 份本人作品 + 8 份课程/案例文档），中文 **169,083 字**，入库 **`files=19 chunks=830`**；
>   第一域未被破坏（`data/raw/` 13 文件、`chunks.jsonl` 仍 400 条 / 12 来源、最后写入仍是 09-18）；**`src/**` 与 `SPEC.md` 零改动**
> - **转换用 Word COM 正规转换**（`.doc/.docx`）+ PDF 直接复制（走 `pypdf`）；转换后**又剔除 1 份主题不符的**
>   （`07` 是思政课报告，"软件工程1班"是班级名）—— **先转换、再按内容核对、后剔除**
> - **T-027**：20 题（12/5/3，id `se01`…）**先于评测单独提交**（`4a6733f`，那次提交里没有任何模型输出或分数）；
>   经独立复核 **9 条 finding** 修订（含 1 条 high：`se06` 同文档 RTO 自相矛盾会**假失败**）
> - **基线（两次运行）**：`accuracy` **0.90 / 0.95**、`partial_rate` 0.10/0.05、`wrong_rate` **0.00**、
>   `refusal_accuracy` **1.00**、`hit_rate` 0.85、`avg_keypoint_coverage` **0.8913 / 0.9109**、`avg_latency_ms` 1578.7 / 1644.1
> - **分题型（第 2 次）**：单跳 **12/0/0**（cov 0.9667）· **多跳 4/1/0（cov 0.7771）** · 不可答 3/0/0；唯一失分题是 `se15`（cov 0.286）
> - ⭐ **"多跳最弱"跨域复现**：第一域多跳覆盖率 0.40~0.55、第二域 **0.7771**，**都明显低于同域单跳** ⇒
>   **多跳的难度更像"题型固有"，而不是第一域（框架文档）语料特有的问题** —— **这是 E-01~E-08 一直缺的那个对照**
> - ⚠️ **两域数字绝对不可比**（必须一起说）：语料风格不同（本人作业含**成串的明确指标与表格**，答案可直接摘抄）+
>   题库构建方式不同（第二域每题都过了"**能贴出连续原文片段**"这一关）。**0.90 与 0.50 的差距主要说明"语料与题目的可答性"，不是"系统变好了"**；
>   但它本身是个可讲的发现：**同一套代码、同一个模型，换语料就从 0.50 到 0.95** ⇒ 报 RAG 准确率必须同时报语料与题库
> - **成本**：T-026 ≈0.1 元 + T-027 **0.6 元**（原估 0.3，**超支 0.3 元**：`run_eval.py` 只写汇总、没有 `--detail-out`，
>   而卡面要求分题型 ⇒ 用**仓库外**一次性脚本复刻同一链路补跑一次；**未改任何仓库代码**）
> - **已知限制**：本域语料**不入公开仓库**（含学号、同学与导师姓名）⇒ **clone 后无法直接复现本域数字**，
>   这是与第一域在可复现性上的**结构性差异**；题库里有 3 题（`se09`/`se10`/`se12`）可凭通用知识答对（检索区分力低）；
>   语料含 **3203 个 BEL 控制字符**（Word 表格行首）未处理。详见 `EXPERIMENTS.md` **E-09**
...
## T-022 · `top_k_final` × rerank 联合实验（多跳瓶颈的下一步）

| | |
|---|---|
| **难度 / 档位** | L1 / T1 |
| **依赖** | T-021（同进程 A/B 的方法与报告格式都已就位） |
| **inScope** | `docs/**`、`README.md`、`tests/test_offline.py`（仅在需要补测时）、`src/**`（**仅在诊断发现真缺陷时**，不预期） |
| **outOfScope** | `SPEC.md`、`app/**`、`eval/questions.jsonl`、`eval/metrics.py`、`data/**` |

**目标**：检验 **H1** —— 「rerank 对多跳无效，是因为窗口只有 4 个席位、它在替多跳题做取舍」。
做法：**先跑 0 元的检索侧机制诊断**（窗口里到底有没有那篇依据），**再**决定是否花约 0.9 元跑 LLM 评测。

**Step 1 · 检索侧机制诊断（0 元 LLM 费用：只花 query embedding + rerank 调用，无任何生成调用）**
对 20 题 × 4 个检索配置 `(k=4,off) (k=8,off) (k=4,on) (k=8,on)`，**不调用 LLM**，记录：
- `gold_sources` 有几篇进了**最终窗口**（这是"依据在不在模型眼前"的直接证据）
- 多跳题**两篇依据是否都进窗口**（E-01 用过的集合级口径）
- 与 `debug()` 的融合池对照，看截断发生在哪一步

**决策点（必须显式记录走了哪条）**
- 若 `(k=8, on)` 的多跳题仍然**只有 1 篇依据进窗口** → **H1 死**，本卡以负结果收尾，**不进入 Step 2**（省下 0.9 元）
- 若两篇都进了 → 进 Step 2（LLM 侧验证"依据都在眼前，模型能不能答对"）

**Step 2 · LLM 侧三格同进程评测（仅当 Step 1 放行；约 0.9 元）**
同一 `Retriever` 实例、只切 `search(..., top_k=?, use_rerank=?)`，三格各 20 题：
- A = `(k=4, off)`（同进程锚点）
- B = `(k=8, off)`（扩大窗口，复验 E-02）
- D = `(k=8, on)`（**H1 的关键格**）
> **为什么必须同进程**：`§5.12` 已证向量层跨进程不可复现；跨进程比会把"窗口/重排的效果"和"召回漂移"混在一起（T-021 已踩过这个坑并解决）。

**通过标准（逐条可执行）**
1. Step 1 的 4 配置 × 20 题窗口覆盖率表**打印出来**（含多跳题逐题"几篇依据进窗口"）
2. 决策点**显式记录**：若走"不进入 Step 2"，必须给出"为什么机制上已经不可能有效"的证据
3. 若进入 Step 2：三格汇总 + **分题型**（`single_hop` / `multi_hop` / `unanswerable`）覆盖率与桶内计数
4. 断言并记录：每次 `search` 实际用了哪个 `top_k`/`use_rerank`（防止"以为改了其实没改"）
5. `& $P -m compileall -q $R` 退出码 0；离线单测全绿（**只增不减**，当前 52）
6. Step 1/2 的结论写进 `docs/EXPERIMENTS.md` **E-05**（**正负都写**；负结果同样算合格交付）
7. `RERANK_ENABLED`、`TOP_K_FINAL` 的**默认值都保持不变**（改默认值须另开卡 + 用户批准）
8. `git status` 不出现 `data/**`、`eval/report*.json`、`.env`

**结论（负结果，2026-09-21）**：**H1 被证伪，Step 2 未执行（省下约 0.9 元）**。
多跳题「两篇依据都进窗口」：`(k=4,off)` **3/5** · `(k=8,off)` **4/5** · `(k=4,on)` **2/5** · **`(k=8,on)` 3/5** ——
H1 预测"扩窗口后 rerank 不再挤依据"，实测反而更差；**最干净的反例 `q15`：8 席且不开 rerank 时三篇全进，一开 rerank 就掉一篇**（**席位足够也照挤**）。
唯一符合 H1 的是 `q14` ⇒ **H1 只在个别题上成立**。详见 `EXPERIMENTS.md` **E-05** 与 `PLAN.md §5.16`。

**成本**：**80 次 query embedding + 40 次 rerank 调用（无任何 LLM/生成调用），约 0.01 元** —— 原计划 Step 2 的 0.9 元**未花**。

**红线**
1. 禁止跳过 Step 1 直接花钱 —— 这是本卡最可能省下 0.9 元的地方
2. 禁止只报总体数字（**E-04 的教训：总体掩盖分题型，目标没达成会被藏起来**）
3. 禁止把"rerank 被降级"的结果当成结论（E-04 的三条证据方法照用）
4. 禁止在 H1 被证伪时硬凑一个"部分有效"的说法 —— **负结果比模糊结论值钱**
5. 禁止顺手改默认值或放宽 `SPEC.md`

**结论（已填，2026-09-21）**

```
任务：T-022 top_k × rerank 联合实验
改动文件：docs/EXPERIMENTS.md、docs/PLAN.md、docs/BOARD.md、README.md、RAG项目_面试问答.md（**src/** 零改动**）
Step 1（0 元）窗口覆盖率表：
  (k=4,off) 多跳"两篇都进窗口" = 3/5     全部依据进窗口 14/17（有依据的 17 题）   平均命中 1.000/20 题
  (k=8,off) = 4/5                        16/17                                    1.100
  (k=4,on)  = 2/5                        14/17                                    1.000
  (k=8,on)  = 3/5   ← H1 关键格          15/17                                    1.050
  多跳文件级连续覆盖率（§2.12 判据的代理）：0.8333 / 0.9000 / 0.7333 / 0.8333 → k8on < k8off，换口径方向不变
  证据：_rerank 调用 40 次 / 异常 0 次；窗口"内容不同"按 `chunk_id` 级 = 20/20（k=4 与 k=8）；
        文件级有三个数：有序 source 列表 13/20 与 19/20、去重集合 10/20 与 12/20（引用时必须写明是列表还是集合）
决策点：**以负结果收尾，不进入 Step 2（省下 0.9 元）**
  理由：H1 预测 (k=8,on) ≥ (k=8,off)，实测 3/5 < 4/5；q15 给出"8 席足够仍被挤掉"的直接反例
        （判据说明：卡面原写的字面阈值是"仍只有 1 篇"，实测 3/5 不是字面触发；
          我按 H1 的预测方向判定 —— 方向更严（提前判定失败），已在 E-05 §4 写明）
Step 2：**未执行**（无 LLM 侧数据）→ 「(k=8,on) 的答案是否好于 (k=4,on)」**仍未测**
H1 判定：**被证伪**（唯一支持例 q14；q15 反例；q13 与两个旋钮都无关）
验收结果：
  [x] compileall 退出码 0 —— 实际：compileall exit=0
  [x] 离线单测全绿（Ran N tests, OK）—— 实际：Ran 52 tests / OK
  [x] Step 1 表已打印 —— 实际：见 E-05 §1（4 配置 × 20 题，含多跳逐题"缺哪一篇"）
  [x] E-05 已写入（含原始数据与分题型）—— 实际：EXPERIMENTS.md「## E-05」§1~§6
  [x] 默认值未改（RERANK_ENABLED=0 / TOP_K_FINAL=4）—— 是（本轮 src/** 零改动）
遗留问题：① `q13` 四种配置全缺同一篇，原因未定位（与窗口/重排无关）；
        ② 「文件级依据在窗口里」≠「需要的那一小节在窗口里」（E-02），小节级覆盖**尚未测量**；
        ③ H1 在 `q14` 上成立 ⇒ "扩窗口 + rerank"不是普遍无效，只是不普遍有效
下一步建议：**先做 0 元的小节级覆盖测量**（把 keypoints 对应到具体 heading，
        看 k=8,off 下多跳题缺的是不是"小节的错"），再决定是否值得花钱；
        另一个正交方向是**按 source 限席位的排序**（保证跨文档覆盖，直击 rerank 挤依据的机制）
```

**成本**：**0 元 LLM 费用**（只需 query embedding 复用现有检索；证据匹配是纯本地文本匹配）

**通过标准（逐条可执行）**
1. **证据可定位率表**：逐题给出「keypoints 总数 / 能匹配到 chunk 的数量 / 匹配到几个不同小节」
2. **覆盖率的四级分解表**（每级都分文件 / 小节 / 块）：`融合池` → `窗口(k=4,off)` → `窗口(k=8,off)`
3. **多跳题逐题表**：每题的 gold 文件、证据小节、以及各级是否命中（要能一眼看出丢在哪一级）
4. **H2 判定**：多跳「小节级全部进窗口」的题数 vs 「文件级全部进窗口」的题数（同配置对比），明确写成立 / 被证伪
5. `& $P -m compileall -q $R` 退出码 0；离线单测全绿（**只增不减**，当前 52）
6. 结论写进 `docs/EXPERIMENTS.md` **E-06**（**正负都写**）
7. `src/**`、`SPEC.md`、所有默认值、题库判分**均不动**
8. `git status` 不出现 `data/**`、`eval/report*.json`、`.env`

**红线**
1. **不得为了让 H2 成立而放宽"证据"的定义** —— 定义在 §2.13 已预注册；要改必须写进 E-06 并说明
2. **不得把"匹配不到的 keypoint"算成未命中**（那会人为压低小节级覆盖率 —— 正是 H2 想要的结果，属自欺）
3. 不得只报多跳、不报其他题型（E-04 的教训：总体 vs 分题型）
4. 不得在 H2 被证伪时含糊收尾 —— **E-01 的第一版结论可能因此被部分平反，这种事要敢写**
5. 禁止顺手改代码或默认值（本卡预期 **`src/**` 零改动**）

## T-024 · `k=12/15` 的 LLM 侧验证（**卡面正文见 §2.14**）

**成本**：Step 1 ≈ **0 元**（prompt 长度可在拿到融合池后**离线算出**，无额外 API）；Step 2 ≈ **0.6 元**（40 次生成）。**上限 0.8 元。**

**通过标准（逐条可执行）**
1. Step 1 的 prompt 长度 / 丢资料表**打印出来**（4 个 `k` × 中位/最大/丢资料题数 + 逐题）
2. **决策点显式记录**：走了哪条分支、依据哪个数（若判"不进入 Step 2"，须给出"为什么"）
3. 若进入 Step 2：**同进程** A/B，汇总 + **分题型**覆盖率与桶内计数，且标注**是否发生截断**
4. 记录每次 `search` 实际使用的 `top_k`（防止"以为改了其实没改"）
5. `& $P -m compileall -q $R` 退出码 0；离线单测全绿（**只增不减**，当前 52）
6. 结论写进 `docs/EXPERIMENTS.md` **E-07**（正负都写）
7. 默认值与 `SPEC.md` **均未改**（给出 `src/config.py` 与 `SPEC.md` 的相关原文行）
8. `git status` 不出现 `data/**`、`eval/report*.json`、`.env`

**红线**
1. **不得为了凑一个"窗口有效"的结论而放宽 `max_chars`**（那是改契约，需另开卡 + 用户批准）
2. 不得只报答案、不报截断（E-03 的教训）；也不得只报覆盖率、不报答案（E-06 的教训）
3. 不得把"覆盖率涨了"直接说成"答案会涨" —— **H3 要的是答案侧证据**
4. H3 被证伪就如实写成负结果（"覆盖率≠答案"会是第二次确认，同样有价值）
5. 禁止顺手改默认值

## T-025 · 仅向量 / 仅 BM25 / 混合（RRF）三路同进程对照（**卡面正文见 §2.15**）

**通过标准（逐条可执行）**
1. 三格的汇总表**打印出来**（主指标 + 分题型 + `hit_rate` + 延迟），并标注**同进程**
2. 检索侧口径表：三路的 `gold_sources` 命中率、仅 BM25 的窗口条数分布
3. **不变式**：仅向量格的窗口顺序 == `_rankings()["vector"][:4]`（逐题断言，0 违反）
4. `& $P -m compileall -q $R` 退出码 0；离线单测全绿（**只增不减**，当前 52）
5. 结论写进 `docs/EXPERIMENTS.md` **E-08**（正负都写，含"简历那句话能不能写"的明确结论）
6. `git status` 不出现 `data/**`、`eval/report*.json`、`.env`；**`src/**` 零改动**（给 `git diff --stat` 证据）
7. 默认值与 `SPEC.md` 均未改

## T-026 · 第二语料域的语料准备与落地（软件工程实践）

| | |
|---|---|
| **难度 / 档位** | L1 / T2（转换）+ T1（入库核对） |
| **inScope** | `data/raw_se/**`、`docs/DOC_SOURCES_SE.md`、`docs/**`、`README.md`、`.env.example`（只加注释说明，不改任何默认值） |
| **outOfScope** | `SPEC.md`、`src/**`、`app/**`、`tests/**`、`eval/**`、**`data/raw/**`（现有语料绝对不动）** |

**目标**：把 §2.16 预注册的 **20 份**文档转成**纯文本 `.md`** 放进 `data/raw_se/`，
用**独立的环境变量**入库成第二套索引，并登记来源 —— 全程**不改 `src/`、不改 `SPEC.md`**。

**通过标准（逐条可执行）**
1. `data/raw_se/` 下 **≥18 份** `.md`/`.txt`/`.pdf`（目标 20 份），**中文正文合计 ≥15 万字**（给实测命令与输出）
2. **语料目录里不残留 `.doc`/`.docx`/`.pptx`/`.xlsx`**（证明全部已转换）
3. 文件名唯一、**无 `README.md`**（`data/raw_se/README.md` 若需要只能作为说明且不会被入库 —— 但**更稳的做法是不建它**）
4. **转换质量抽检**：抽 3 份，打印各自前 300 字，**必须通顺中文**（不得出现 `m..m..` 之类交错乱码）
5. 入库成功且**用的是独立索引**：`RAW_DIR=data/raw_se` + `CHROMA_DIR=data/chroma_se` + `CHUNKS_FILE=data/chunks_se.jsonl` + `COLLECTION=doc_qa_se`，
   打印 `files=20 chunks=M`，且 **300 ≤ M ≤ 1200**（防切块失控）
6. **现有索引与会话状态未被破坏**：`data/raw/` 仍 12 份、`data/chunks.jsonl` 与 `data/chroma/` 未被改写（给修改时间/哈希证据）
7. `docs/DOC_SOURCES_SE.md` 登记每份：原始路径 / 转换方式（Word COM 或 pypdf）/ 中文字数 / sha256 前缀
8. `& $P -m compileall -q $R` 退出码 0；离线单测全绿（当前 52）
9. **`src/**` 零改动**、`SPEC.md` 未改、默认值未改（给 `git diff --stat` 证据）
10. `git status` 不出现 `data/**`（语料与索引均在 `.gitignore` 内）、`eval/report*.json`、`.env`

**成本**：**约 0.2 元**（只有入库时的 embedding；17 万字 ≈ 25 万 token；无任何生成调用）

**红线**
1. **禁止用"读字节 + 正则"的方式转 `.doc`** —— 本机 Word COM 可用，必须用正规转换（普查时已见乱码交错的样本）
2. **禁止把重复文档塞进语料**（同名 PDF/DOCX 只留一份；`第3组 PBL` 的 pptx 与 doc 只留一份）
3. **禁止改动 `data/raw/` 及其索引** —— 现有 20 题与全部实验资产都依赖它
4. 禁止为凑字数塞入碎片、引文列表、说明文件
5. 禁止改 `SPEC.md` 或 `src/**`（若发现必须改代码才能做，**停下来开新卡**）
...
| **难度 / 档位** | L1 / T1 |
| **依赖** | T-026（语料必须已入库） |
| **inScope** | `eval/questions_se.jsonl`、`docs/**`、`README.md` |
| **outOfScope** | `SPEC.md`、`src/**`、`app/**`、`tests/**`、`eval/questions.jsonl`（**现有题库绝不改**）、`eval/metrics.py`、`data/**` |

**目标**：为新域出一套 20 题题库，并跑出**新域的第一个真实数字**。

**通过标准（逐条可执行）**
1. `eval/questions_se.jsonl` **20 行**，字段与 `eval/README.md §2` 一致，`id` 唯一（**用 `se01`…`se20` 前缀，避免与现有题库 `q01`… 冲突**）
2. 配比 **12 单跳 / 5 多跳 / 3 不可答**，与 `eval/README.md §1` 一致
3. **`gold_sources` 全部是 `data/raw_se/` 下真实存在的文件名**（脚本逐条校验，输出证据；写错等于自动判错）
4. `multi_hop` 每题 **≥2 篇不同文档**；`unanswerable` 3 题的**依据确实不在新语料里**（逐题说明问的是什么、为什么没有）
5. 无 `\uXXXX` 转义（中文直写）；每题 `keypoints` ≥2
6. **独立复核**：另派角色逐题核"标准答案能否由 `gold_sources` 那个文件支撑"，findings 必须处理完
7. 真实评测跑通：`eval/report_se.json` 生成，`n=20`，且**必须分题型报**（`single_hop`/`multi_hop`/`unanswerable`）+ `avg_keypoint_coverage`
8. 结论写进 `docs/EXPERIMENTS.md` **E-09**，且**必须写明"新域数字与旧域数字不可比"**（语料不同、题库不同）
9. `compileall` 0 + 离线单测全绿；**`src/**` 零改动**、`SPEC.md` 与现有题库未改
10. `git status` 不出现 `data/**`、`eval/report*.json`、`.env`

**成本**：**约 0.3 元**（20 题一次真实评测）

**红线**
1. **禁止修改 `eval/questions.jsonl`**（现有题库定稿冻结；`eval/README.md §3.5`）
2. 禁止把"新域 0.6"直接与"旧域 0.6"比较 —— 语料、题库、难度都不同
3. 出题不得指向语料外的依据（`gold_sources` 写错等于判错）
4. 禁止为了数字好看挑简单的题（配比与题型必须按规范）
...
14. **小节级整理（`SECTION_MODE`）实测无效，默认保持 `off`（T-020，2026-09-21）**：新增的环境变量 `SECTION_MODE` 有三档 —— `off`（默认，行为与改动前逐字一致）/ `diverse`（同一小节只留最高分块）/ `expand`（命中块所在小节整节进候选）。
     ...
     | prompt 中位 / 最大（字符） | 1623.5 / 1936 | 1465 / 1936 | **2388 / 4742** |
...
16. **`top_k` × rerank 联合：假设被证伪；0 元的机制诊断省下 0.9 元（T-022，2026-09-21）**：
    E-02 说"扩大窗口对多跳有效"、E-04 说"重排对多跳无效"，于是提出 **H1**：rerank 是在**替多跳题做取舍**（4 个席位不够），
    扩到 8 席它就不必排除、多跳可能因此受益。
    **实测（无任何 LLM/生成调用；80 次 query embedding + 40 次 rerank 调用，约 0.01 元）**——多跳题「两篇依据都进窗口」：

    | 配置 | 多跳两篇都进<br>(/5) | **有依据的 17 题中全部依据都进窗口**<br>(/17) | 平均命中依据数<br>(/20 题) |
    |---|---|---|---|
    ...
    > **分母必须写清**：3 道不可答题 `gold_sources` 为空，"全部依据都进窗口"对它们是**空集恒真**，
    > 所以该行**只统计有依据的 17 题**；若统一按 20 题分母则为 **17 / 19 / 17 / 18**。
    > **两个数都对，写清分母比写哪一个数重要** —— 独立验证就是在这里抓到过一处口径错误（见 `EXPERIMENTS.md` E-05 §7）。
    >
    > **§2.12 预注册判据的代理**：多跳**文件级连续覆盖率** `k=4,off` 0.8333 · `k=8,off` **0.9000** · `k=4,on` 0.7333 · `k=8,on` **0.8333**
    > ⇒ `k8on < k8off`，**换口径方向不变**

    - ⚠️ **H1 预测 `(k=8,on) ≥ (k=8,off)`，实测低于（3/5 < 4/5）**
    - **最干净的反例 `q15`**：`k=8` 且**不开** rerank 时三篇依据全进窗口；**一开 rerank 就掉一篇** ⇒
      **席位足够也没用 ⇒ 挤依据不是"位置不够"，而是 rerank 的目标函数（单 query 相关性）与"跨文档互补覆盖"本来就不一致**
    - 唯一符合 H1 的是 `q14`（扩窗口确实救回来了）⇒ **H1 不是全错，而是"只在个别题上成立"**
    - `q13` 四种配置全缺同一篇 ⇒ 这类题与**窗口/重排都无关**，问题在融合池排名
    - **顺带解释 E-04**：rerank **从不通过"增加依据覆盖"生效**（`k=4` 开它前后平均命中依据数都是 1.000，可 `accuracy` 却升了；
      `k=8` 开它反而让覆盖变差 1.100 → 1.050）⇒ **它做的是"把已有的块排到更前面"，不是"把缺的块找回来"** ⇒ 单跳受益、多跳受害
    - **怎么引用**：说「我验证了"扩窗口 + rerank"这个假设，**被证伪**；rerank 挤依据不是因为席位不够」。
      **不要**说"已证明 k=8 + rerank 没有价值" —— 本卡**没有 LLM 侧数据**，
      「`(k=8,on)` 的**答案**是否好于 `(k=4,on)`」**仍未测**（严格讲：机制失败 ≠ 答案一定更差，E-04 已证 rerank 能靠排序提升单跳）
    - **派生发现（未开卡）**：`k=8,off` 下多跳题已经 **4/5 文件级齐全**，答案却仍然差 ⇒
      **"文件在窗口里"≠"需要的那一小节在窗口里"**（E-02 的结论）。**下一步该量的是「小节级」覆盖** —— 这是 0 元的测量，比再花钱更该先做

17. **小节级覆盖的量化：H2 成立，多跳有四个病因，且"扩大窗口"远没到头（T-023，2026-09-21，0 元）** ⇒ ⚠️ 本条的**最后半句已被 T-024 推翻**（数据成立、结论不成立），见下方 `§5.18` 与 `EXPERIMENTS.md` E-07：
    证据定义与判据**在跑之前就预注册并单独提交**（`PLAN.md §2.13`，commit `5c33943`），执行期间**无调整**。
    **多跳（5 题）**：文件级 **4/5** → **小节级 2/5** → 块级 **1/5**（`k=8, off`）；`k=4, off` 为 3/5 · 2/5 · 1/5。
    - **H2 成立**：**窗口里有那份文件、但没有那一节**（小节级未过 4 题：`q13`/`q14`/`q16`/`q17`；其中因"整节没进窗口"失分 3 题：`q13`/`q14`/`q17`，`q16` 属③）—— E-02 的定性结论第一次被量化
    - ⚠️ **我自己推翻了一条判断**：原写"扩大窗口这条线到头了"，**错**。把 `k` 扫全：多跳小节级 `k=4` 2/5 → `k=8` 2/5 → **`k=12` 3/5 → `k=15` 4/5**；块级 `k=8` 1/5 → **`k=12` 3/5**。
      **需要的小节常排在第 9~15 名**，而 E-02/E-05 只比较了 4 与 8，**正好卡在拐点之前** ⇒ 当时把 `top_k` 放回第一候选
      🔴 **但这条"放回候选"随后被 T-024/E-07 否掉**：窗口里的东西**到不了模型**（`k=8/12/15` 的 prompt 逐字相同）⇒ **真正的杠杆是 `max_chars`**
    - **四种病因（多跳 20 个 keypoint，`k=8,off`）**：
      **① 池子里就没有 3/20**（该 keypoint 的**全部**证据块都不在池里 ⇒ 召回/融合层）· **② 进了池子但节没进窗口 4/20**（排序/截断）·
      **③ 小节在、块不在 2/20**（窗口内块选择）· **④ 全都在场 11/20**（若仍答错 ⇒ 生成/综合）
    - **这改变了下一步的优先级**：过去四张卡（E-02/E-03/E-04/E-05）一直在 **②** 上打转，而它只占 **4/20**；
      **①（召回层）第一次被量化**成 3/20，而它此前被"文件级 5/5"这个口径完全掩盖
    - **E-01 的第一版结论被部分平反**：`q15` 的四个要点的证据块**都至少有一块在窗口里**，且它答对了（1.000）；
      `q16` 也 3/4 在场却只拿 0.500 ⇒ **"东西都在眼前之后仍答不好"确实存在**（④ 占 11/20）。
      当年 E-01 说"瓶颈在生成侧"、被 E-02 用指标粒度推翻 —— 现在看：**它错在把这当成唯一原因，而不是错在指出了它**
    - **怎么引用**：说「我把口径从文件降到小节，多跳从 4/5 掉到 2/5，说明**有文件、没那节**；再降到块只剩 1/5。
      拆开后发现多跳不是**一个**瓶颈，**召回 3/20、排序 4/20、块选择 2/20、生成 11/20**」。
      **不要**说"我找到了多跳的原因"（这是**分布**，不是单一原因）·**不要**用本卡推断 `accuracy`（本卡没跑生成）
    - **口径警告**：证据靠**词面子串匹配**定位，通用 keypoint 会放宽小节级（`q16` 的 `.vue` 命中 52 个小节）**与块级**（141 个块）判定 ——
      两个方向的偏差**都对 H2 不利**，所以 H2 成立不依赖它；但**不能**据此说"小节级容易满足"，也不能把"块级 1/5"当成精确值（它是**保守下界**）
    - **独立验证抓到我 7 处问题，全部已改**（1 处 high：① 的规则文本与算出的数不自洽，数字未变、改的是文字；
      1 处 high：README 一处旧文与新结论冲突；另 5 处措辞/口径）——
      且验证线索触发我把 `k` 扫全，**推翻了我自己"窗口到头了"的判断**。详见 `EXPERIMENTS.md` E-06 §7
    - **未做的事（都属新卡，按优先级）**：**① 先验 `k=12/15`**（§3.5 证仍有效，0.3 元级；**必须同时报 prompt 长度与截断题数**）·
      治 ① 召回 · 治 ④ 生成（按小节分组逐节回答 / 子问题拆解，**不需要改检索**）· 治 ③ 窗口内块选择

18. **加大窗口无效：`k≥8` 之后模型看到的内容完全一样；真正的约束是 `max_chars`（T-024，2026-09-21，0 元）**：
    E-06 §3.5 显示多跳覆盖率随 `k` 上升（小节级 `k=12` 3/5、`k=15` 4/5），据此本卡提出 **H3**：这个提升能传导到答案。
    **判据先于结果**（`PLAN.md §2.14`，commit `575c3d0`），执行期间**无调整**。

    | `k` | prompt 中位 | 被 3000 预算丢资料的题数 | prompt 里实际保留（中位） | **多跳「要点证据都进 prompt」** |
    |---|---|---|---|---|
    ...
    - **H3 被证伪，而且不是"效果小"**：`k=8/12/15` 送给模型的 prompt **逐字相同** —— 加窗口只是多取候选，`build_prompt` 随后整条丢掉
    - **预注册决策点生效**：`k=12`/`k=15` 都丢 20/20 > 阈值 4/20 ⇒ **不进入花钱的 Step 2**（省下约 0.6 元）。
      实际触发信号（顶到 2800~2999 天花板）**比阈值强得多**，所以这个判定不依赖阈值选得准不准
    - **同时推翻 E-06 §3.5 的"结论"部分**（数据仍成立）：那里说"`top_k` 回到第一候选"，实际 **`k≥8` 后 `top_k` 对模型零影响**
    - **这解释了此前的几处矛盾**：`k=4→8` 有效（E-02）是因为 prompt 还在长（1623.5→2800、4→7 条）；8 之后被截断。
      rerank 只对单跳有效（E-04），是因为多跳需要的"第二篇"常排在第 8 名之后 —— **排到哪儿都没用，模型看不见**
    - **唯一剩下的突破口：`max_chars`（`SPEC.md §5` 冻结值 3000）**。多跳 5 题里 4 题的关键要点在第 4 条资料之后才出现，而 prompt 在 3000 字符处硬停。
      ⚠️ **改它要动 `SPEC.md`，必须先取得用户批准**
    - **怎么引用**：说「我验证过加大窗口，**在花钱之前就用 prompt 预算证明了它无效**：`k=8` 之后模型看到的内容逐字相同。
      真正的约束是 prompt 的 3000 字符预算，而这属于契约，需要走变更流程」
    - **这也是「粒度决定你能看见什么」的第五次现身**：文件级 → 小节级 → 块级 → 窗口级 → **prompt 级**；
      前四级都在"检索侧"，而**模型只认最后一级** —— 前四级涨、最后一级不动，等于没涨

19. **三路检索对照：混合召回反而比仅向量差 0.093（T-025，2026-09-21，约 0.9 元）**：
    这张卡补的是《准备清单》09-22 那条"优化前后对比表"（简历里"准确率从 ___%（仅向量）提升至 ___%"）。
    **判据先于结果**（`PLAN.md §2.15`，commit `36543ec`），执行期间无调整，**`src/**` 零改动**。
    ...
    - **H4 被证伪**：混合比仅向量低 **0.05~0.09**（独立验证复现区间；逐题升 2 / 降 4 / 平 14），`accuracy` 差 0.05~0.10 —— **远超噪声带**（`§5.5` ±0.01~0.02）
    - **分题型揭示机制**：单跳 **仅向量 0.7222** vs 混合 0.5694 vs 仅 BM25 0.5417；多跳 **仅 BM25 0.5000** vs 混合 0.4500 vs 仅向量 0.4000
      ⇒ **两路确实各有擅场，但固定权重的 RRF 等权融合把弱的那一路也平均了进来**
    - **层级教训（第六次），且我初版选错了口径**：检索侧**四个口径方向不同** —— `hit_at_k`（任一命中）**17/16/16** 同向 ·
      **"全部 gold 文件进窗口" 13/14/14 反向**（分母 = 有 gold 的 17 题）· 证据**小节**级 **11/8/9** 同向 · 证据**块**级 **7/6/6** 同向。
      **只有文件级那一个口径反向** ⇒ **正确的机制说法是「混合让"需要的证据"更难进窗口」**（初版"文件更全、块质量更差"**方向说反了**，已改）
      **检索侧混合 ≥ 向量，答案侧混合 < 向量**：窗口里文件更全，但块级证据质量更差
    - **简历必须改写**（判据第 7 条）：**不能**写"混合召回提升了准确率"；**可以**写"实现两路 + RRF，自建评测后实测混合反而低于仅向量（0.534 vs 0.627），
      分题型看单跳向量强、多跳 BM25 强，说明固定权重融合不是最优"
    - **⚠️ 不据此改默认值**：仅向量的 `accuracy 0.60` 是本项目测到过的最好数字，但**单次运行、n=20**；改 `SPEC.md` 的设计要另开卡 + 多轮复跑 + 用户批准
    - **怎么引用**：说「我把三种检索路做了同进程对照，**结论和我们最初的设计预期相反**：混合比仅向量差 0.093；
      两路各有擅场（单跳向量、多跳 BM25），所以问题在**融合方式**而不是融合与否」
    - **下一个实验（未开卡）**：两路不同权重 · 按题型路由 · RRF `k` 调参 · **以及"仅向量 + rerank"**（rerank 的 +0.088 是在混合基线上测的，换基线是否仍成立未知）

20. **第二语料域的落地，与一处「超出卡面但必须」的改动（T-026，2026-09-21，约 0.1 元）**：
    - **落地结果**：`data/raw_se/` **19 份**（11 份本人作品 + 8 份课程/案例文档），中文 **169,083 字**；入库 **`files=19 chunks=830`**；
      独立索引 `COLLECTION=doc_qa_se` / `CHROMA_DIR=data/chroma_se` / `CHUNKS_FILE=data/chunks_se.jsonl`；**`src/**` 与 `SPEC.md` 零改动**
    - **第一域未被破坏**：`data/raw/` 仍 13 个文件；`data/chunks.jsonl` 最后写入时间仍是 **2026-09-18**、仍 **400 条 / 12 来源**；两域来源文件名**零重叠**
    - ⚠️ **改动了一处 outOfScope 文件**：`.gitignore`（T-026 卡面 inScope 里**没有它**）。
      **为什么必须改**：原规则 `data/raw/*` **不覆盖 `data/raw_se/`** ⇒ 第二域语料会被提交进**公开仓库**，
      而它含**学号、同学与导师姓名**（隐私）以及课程/案例材料（第三方版权）。已加 `data/raw_se/` · `data/chroma_se/` · `data/chunks_se.jsonl` 三条，
      并在 `README.md`「第二语料域」与 `DOC_SOURCES_SE.md §四.3` 写明"clone 后无法直接跑第二域"。**按 T-004/T-014 的先例显式登记，不以"顺手"名义越界**
    - **语料卫生（T-012 教训的延续）**：转换完成后**又剔除 1 份**（`07_软件工程课题研究报告.md`）——
      文件名里的"软件工程 1 班"是**班级名**，内容实为**思政课**的"实事求是"课题研究报告。
      **先转换、再按内容核对、后剔除**，而不是只看文件名就收下
    - **已知限制**：Word 转换丢表格结构与图片、目录页码留在正文（**未额外清洗** —— 将来做语料清洗对照时这是天然 before 状态）；
      本域平均块长 ≈204 字（标题硬边界所致），明显短于第一域 ≈267 字

21. **第二语料域的基线：0.90~0.95，且"多跳最弱"跨域复现（T-027，2026-09-21，0.6 元）**：
    - **结果**：`accuracy` **0.90 / 0.95**（两次运行）、`partial_rate` 0.10/0.05、`wrong_rate` **0.00**、`refusal_accuracy` **1.00**、
      `hit_rate` 0.85、`avg_keypoint_coverage` **0.8913 / 0.9109**、`avg_latency_ms` 1578.7 / 1644.1
    - **分题型**：单跳 **12/0/0**（cov 0.9667）· **多跳 4/1/0（cov 0.7771）** · 不可答 3/0/0（唯一失分题 `se15`，cov 0.286）
    - ⭐ **跨域对照（本项目此前缺的那一块）**：**两个域都是"多跳最弱"**（第一域多跳 cov 0.40~0.55、第二域 0.7771，均低于同域单跳）⇒
      **多跳的难度更像"题型固有"**，不是第一域语料特有的问题；且第二域多跳**检索 5/5 命中**却仍有 1 题部分对 ⇒ 再次指向"检索到了 ≠ 答得好"
    - ⚠️ **两域数字绝对不可比**：语料风格（第二域是本人作业，含成串明确指标与表格，答案可直接摘抄）+ 题库构建方式
      （第二域每题都过了"能在原文贴出连续片段"这一关）都不同 ⇒ **0.90 vs 0.50 主要说明"语料与题目的可答性"，不是"系统变好了"**
    - **怎么引用**：说「我在**两套语料域**上各建了一套 20 题评测：框架官方文档域 **0.50**、我自己的课程项目文档域 **0.90~0.95**。
      **差距主要来自语料的信息密度与问答锚点明确度**，同一套代码同一个模型。**两个域的共同点是多跳都最弱**」
    - **不要**说"我把准确率从 50% 优化到了 95%" —— 那是**换了语料和题库**，不是优化
    - **超支登记**：本卡实花 **0.6 元**（估 0.3）。原因：`eval/run_eval.py` 只写汇总、没有 `--detail-out`，
      而卡面验收要求**分题型** ⇒ 用**仓库外**脚本复刻同一链路补跑一次。**未改仓库代码**；
      若要让"分题型"成为默认能力，应另开卡给 `run_eval` 加明细输出（属 `eval/**` 代码改动）
    - **结构性限制**：本域语料**不入公开仓库**（学号、同学与导师姓名）⇒ **clone 后无法复现本域数字**；
      题库 3 题（`se09`/`se10`/`se12`）可凭通用知识答对；语料含 3203 个 BEL 字符未处理
...
| 2026-09-21 | T-021 收尾自查：**按题型拆开发现 rerank 的收益全在单跳、多跳零效果** → 把"证据链收口"改为"只收到一半" | 总体 +0.088 看起来是成功，但 `single_hop` +0.125、`multi_hop` **±0.000**（桶内 1/3/1 完全没变，`q16` +0.25 被 `q14` −0.25 精确抵消）。**本卡动机是多跳** ⇒ 只报总体数字就是用成功的外壳掩盖没达成的目标。记为「粒度决定你能看见什么」的**第三次**重演 |
| 2026-09-21 | 新增 **T-025**（三路检索对照，M13）：**判据先于结果**（commit `36543ec`），结果 **H4 被证伪 —— 混合召回反而比仅向量差 0.093**，**这是本项目第一次否掉自己的核心设计** | 《准备清单》09-22 要求"加 BM25 混合召回 + Rerank，同一套 20 题重跑，产出优化前后对比表"，并把"准确率从 ___%（仅向量）提升至 ___%"写进简历文案；但**仅向量基线从来没跑过**（无报告、无开关）—— 正是清单**坑 8** 警告的情形。**实测**：三格同进程、唯一自变量 = 用哪几路做 RRF ⇒ 仅向量 **0.6275** · 仅 BM25 0.5294 · 混合 **0.5343**；逐题升 2 / **降 4**。**分题型揭示机制**：单跳仅向量最强（0.7222）、**多跳 BM25 最强（0.5000）** ⇒ **两路互补是真的，等权 RRF 融合不是好的组合方式**。**检索侧混合 ≥ 向量而答案侧混合 < 向量**（13/14/14 vs 答案侧反向）——层级教训第六次。**简历改写**：不能写"混合提升了准确率"，改写成"实测混合反而更低、两路各有擅场、问题在融合方式"。**不改默认值**（单次运行、n=20；改契约需另开卡 + 用户批准） |
| 2026-09-21 | 新增 **T-024**（加大窗口的 LLM 侧验证，M12）；**判据先于结果、单独提交**（commit `575c3d0`），结果 **H3 在花钱前被证伪**（省下 0.6 元），**并再次推翻 E-06 §3.5 的结论部分** | E-06 §3.5 说窗口越大检索侧覆盖越高（多跳小节级 `k=12` 3/5、`k=15` 4/5），据此问"这能不能传导到答案"。**Step 1（0 元）**：`k=4/8/12/15` 的 prompt 中位 = **1623.5/2800/2800/2800**、丢资料题数 = **0/15/20/20** ⇒ 预注册决策点直接判"不花钱"。**决定性的一级是新增的"prompt 级"**：`k=8/12/15` 送给模型的 prompt **逐字相同**（都被 `max_chars=3000` 截在 7 条资料），多跳「要点证据都进 prompt」在四个 `k` 上**恒为 1/5**。⇒ **不是"效果小"，是"输入根本没变"**；窗口不是杠杆，**`max_chars` 才是**（`SPEC.md` 冻结值，改需用户批准）。这解释了 `k=4→8` 为何有效（那时 prompt 还在长）而 8 之后无效，也解释了 rerank 为何只对单跳有效。**这是"粒度决定你能看见什么"的第五次现身**：文件→小节→块→窗口→**prompt**，而模型只认最后一级 |
| 2026-09-21 | 新增 **T-023**（小节级覆盖的量化，M11）；**判据先于结果、单独提交**（commit `5c33943`），结果 **H2 成立**、拆出多跳的四种病因，**并自行推翻了一条判断（"窗口到头了"）** | E-02 只有**定性**证据（"缺的内容全在语料里"），E-05 又说 `k=8` 时多跳已 **4/5 文件级齐全**却仍答不好 ⇒ 提出 H2：把口径降到小节 / 块后覆盖率会明显更低。**实测（0 元）**：多跳 文件级 4/5 → **小节级 2/5** → 块级 1/5。**副产品一**：多跳 20 个 keypoint 拆成 **①池子里就没有 3 · ②进了池子没进窗口 4 · ③小节在块不在 2 · ④全在场 11** ⇒ 过去四张卡一直在只占 **4/20** 的 ② 上打转，**① 召回层第一次被量化**；`q15` 全在场且答对、`q16` 3/4 在场只得 0.5 ⇒ **E-01 的第一版结论被部分平反**（它错在当成唯一原因）。**副产品二（自推翻）**：我原写"扩大窗口这条线到头了"，独立验证的线索让我把 `k` 扫全 —— 多跳小节级 `k=12` **3/5**、`k=15` **4/5**，块级 `k=12` **3/5**，**规律单调上升**；需要的小节常排第 **9~15** 名，而 E-02/E-05 只比较了 4 与 8，**正好卡在拐点之前** ⇒ `top_k` 回到第一候选。**教训：只扫两个点就宣布"某条线到头了"，等于用两个样本画曲线** |
| 2026-09-21 | 新增 **T-022**（`top_k` × rerank 联合，M10），**先跑 0 元机制诊断**，结果**假设被证伪、Step 2 未执行（省下 0.9 元）** | E-02 说"扩大窗口对多跳有效"、E-04 说"重排对多跳无效"，交叉出 **H1**：rerank 是因为 4 个席位不够才挤掉依据。**机制诊断（0 元）**：多跳「两篇依据都进窗口」`(k=4,off)` 3/5 → `(k=8,off)` 4/5 → `(k=4,on)` 2/5 → **`(k=8,on)` 3/5** —— H1 预测应 ≥4/5，实测更低；**`q15` 是最干净的反例：8 席足够装三篇，一开 rerank 就掉一篇** ⇒ **挤依据与席位无关，是 rerank 的目标函数与"跨文档互补覆盖"不一致**。**卡面原写的字面阈值（"仍只有 1 篇"）实测未触发，我按 H1 的预测方向判定（方向更严），并在 E-05 §4 写明** —— 判据的调整必须留痕，否则就是"事后改标准"。副产品：rerank **从不通过增加覆盖生效**（k=4 前后平均命中都是 1.000 却提升了答案；k=8 开它反让覆盖变差），正好解释 E-04 的分题型结果 |

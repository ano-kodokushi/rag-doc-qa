...
**`+0.088` 的收益全部来自单跳题。** 也就是说：**排序是全库的瓶颈之一，但不是多跳的瓶颈** —— 多跳的瓶颈至今未定位（已排除：文件级召回、**窗口大小**（`k=4→8` 不足；**但 `k=12/15` 又把它救回来了，见 `docs/EXPERIMENTS.md` E-06 §3.5**）、材料排列、全局重排）。**这两个数必须一起引用。**
...
6. **`multi_hop` 至今没有解，但它的病因已经拆开了**：试过按小节整理上下文（无净收益）、rerank（**对多跳零效果**）、
   ...
   **① 池子里就没有 3/20 · ② 进了池子但节没进窗口 4/20 · ③ 小节在块不在 2/20 · ④ 全都在场 11/20**。
   顺带纠正一条我写错的判断：**"扩大窗口已经到头"是错的** —— `k` 从 8 加到 12/15 仍能把多跳小节级从 2/5 拉到 3/5~4/5、块级从 1/5 拉到 3/5（需要的小节常排在第 9~15 名）。
   所以**不要说"多跳问题已解决"**，也**不要说"是某一个原因"**（详见 `docs/EXPERIMENTS.md` E-02~E-06）。

...
**第二域基线（T-027，两次运行）**：`accuracy` **0.90 / 0.95**、`wrong_rate` **0.00**、`refusal_accuracy` **1.00**、
...
⭐ **跨域对照（本项目此前缺的那一块）**：**两个域都是"多跳最弱"**（第一域多跳覆盖率 0.40~0.55、第二域 0.7771，都低于同域单跳）
⇒ **多跳的难度更像"题型固有"，不是框架文档语料特有的问题**。

⚠️ **两域数字绝对不可比**：第二域是本人课程作业，含**成串的明确指标与表格**（"≤3 秒""500 并发""RTO≤30 分钟"），答案可直接摘抄；
且题库构建时每题都过了"**能贴出连续原文片段**"这一关。⇒ **0.90 与 0.50 的差距主要说明"语料与题目的可答性"，不是"系统变好了"**。
**不要写成"把准确率从 50% 优化到 95%"** —— 那是换了语料和题库。（详见 `docs/EXPERIMENTS.md` **E-09**）

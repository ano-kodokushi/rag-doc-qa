...
> ⚠️ **关于"混合检索"的实测结论（T-025，2026-09-21）**：两路召回（向量 + BM25 → RRF）**确实实现了**，但**同进程三路对照显示：混合反而比仅向量差** ——
> `avg_keypoint_coverage` **仅向量 0.6275 / 仅 BM25 0.5294 / 混合 0.5343**（`accuracy` 0.60 / 0.45 / 0.50，逐题升 2 降 4）。
> 分题型看两路**各有擅场**（单跳向量 0.7222 vs BM25 0.5417；多跳 BM25 0.5000 vs 向量 0.4000）⇒ **问题在"融合方式"而不是"要不要融合"**。
> **因此本仓库不宣称"混合检索提升了准确率"**（详见 `docs/EXPERIMENTS.md` E-08）。
...
⚠️ **但按题型拆开看：rerank 对多跳题「零效果」** —— `single_hop` 覆盖率 0.5694 → **0.6944**，而 `multi_hop` **0.5000 → 0.5000**（桶内 1/3/1 完全没变，`q16` 升 0.25 被 `q14` 降 0.25 精确抵消）。
**`+0.088` 的收益全部来自单跳题。** 也就是说：**排序是全库的瓶颈之一，但不是多跳的瓶颈** —— 多跳的瓶颈至今未定位（已排除：文件级召回、**窗口大小**（`k=4→8` 不足；`k=12/15` 虽能提高检索侧覆盖**但内容到不了模型**，见 E-06 §3.5 与 E-07）、材料排列、全局重排）。**这两个数必须一起引用。**

**三种检索路的同进程对照（T-025）**

| 指标 | **仅向量** | 仅 BM25 | **混合（本仓库默认链路）** |
|---|---|---|---|
| `avg_keypoint_coverage` | **0.6275** | 0.5294 | **0.5343** |
| `accuracy` | **0.60** | 0.45 | 0.50 |
| `hit_rate` | **0.85** | 0.80 | 0.80 |
| 单跳覆盖率 | **0.7222** | 0.5417 | 0.5694 |
| 多跳覆盖率 | 0.4000 | **0.5000** | 0.4500 |

⇒ **混合比仅向量低 0.05~0.09**（逐题升 2 / 降 4；差值不稳、方向稳），**两路各有擅场但等权 RRF 不是好的组合方式**。
⚠️ 单次运行、n=20，**不据此改默认链路**；`accuracy 0.60`（仅向量）是本项目测到过的最好数字，但需多轮复跑才能下结论。
...
2. **`multi_hop` 是短板，而且它的病因是「一个分布」，不是单一原因**：把口径从「文件」降到「小节」「块」之后（T-023，0 元），多跳题在 `top_k_final=8` 时是
   **文件级 4/5 → 小节级 2/5 → 块级 1/5** —— 也就是说**有那份文件、没有那一节**。
   把 20 个要点逐个归类：**① 连融合池都没进 3/20 · ② 进了池子但节没进窗口 4/20 · ③ 小节在、块不在 2/20 · ④ 全都在场 11/20**。
   ⇒ **说明检索侧（① + ② + ③ = 9/20）和生成侧（④ = 11/20）各占一部分** —— 既不能说"瓶颈在生成侧"，也不能说"只是检索的问题"。
   我在这条上**改过两次结论**：最初据桶指标判定「检索侧无效、瓶颈在生成侧」，改用**连续要点覆盖率**重读后发现候选 4→8 让覆盖率**逐题 5 升 0 降**（多跳 0.450 → 0.550），
   于是判「检索侧有效」；再往下拆到小节 / 块，才发现**两种原因同时存在**。复盘见 `docs/EXPERIMENTS.md` E-01 → E-02 → E-06。
...
6. **`multi_hop` 至今没有解，但它的病因已经拆开了**：试过按小节整理上下文（无净收益）、rerank（**对多跳零效果**）、   **「扩窗口 + rerank」联合**（0 元机制诊断即证伪：**8 个席位下 rerank 照样会把多跳依据挤掉一篇**）。
   **把口径从「文件」降到「小节」「块」之后**（T-023，0 元）：多跳 **文件级 4/5 → 小节级 2/5 → 块级 1/5**
   ⇒ 「**有那份文件、没有那一节**」；再按 keypoint 归类，多跳的失败是**四种病因的分布**：
   **① 池子里就没有 3/20 · ② 进了池子但节没进窗口 4/20 · ③ 小节在块不在 2/20 · ④ 全都在场 11/20**。
   顺带纠正**两条**我写错的判断：**"扩大窗口到头了"是错的**（`k` 从 8 加到 12/15，多跳小节级仍从 2/5 升到 3/5~4/5）；
   但 **"那就该调 `top_k`"同样是错的** —— 加了窗口以后 `build_prompt` 会把多出来的资料**整条丢掉**：
   `k=8/12/15` 送给模型的 prompt **逐字相同**（都被 `max_chars=3000` 截在 7 条资料），
   多跳「要点证据真的进了 prompt」在四个 `k` 上**恒为 1/5**。⇒ **真正的约束是 prompt 的 3000 字符预算**（`SPEC.md §5` 的冻结值），不是窗口。
   所以**不要说"多跳问题已解决"**，也**不要说"是某一个原因"**（详见 `docs/EXPERIMENTS.md` E-02~E-08）。
7. **本仓库的"混合检索"是设计选择，不是已验证的优化**：同进程三路对照（T-025）显示**混合比仅向量低 0.05~0.09**（见上方「三种检索路的同进程对照」）。
   所以可以说"**实现了**向量 + BM25 两路召回与 RRF 融合"，**不能说"混合提升了准确率"** —— 后者已被自己的数据否掉。
   下一个该做的是**融合方式**（两路不同权重 / 按题型路由 / RRF `k` 调参），以及"**仅向量 + rerank**"（rerank 的 +0.088 是在混合基线上测的，换基线是否成立未知）。

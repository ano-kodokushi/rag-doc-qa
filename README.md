...
**`+0.088` 的收益全部来自单跳题。** 也就是说：**排序是全库的瓶颈之一，但不是多跳的瓶颈** —— 多跳的瓶颈至今未定位（已排除：文件级召回、窗口大小、材料排列、全局重排）。**这两个数必须一起引用。**
...
6. **`multi_hop` 至今没有解**：试过扩大窗口（有效但没跨过判分阈值）、按小节整理上下文（无净收益）、rerank（**对多跳零效果**）、
   **「扩窗口 + rerank」联合**（0 元机制诊断即证伪：**8 个席位下 rerank 照样会把多跳依据挤掉一篇**，所以"席位不够"不是原因）——
   四条路都排除掉了，所以**不要说"多跳问题已解决"**（详见上面 rerank 段落与 `docs/EXPERIMENTS.md` E-02/E-03/E-04/E-05）。

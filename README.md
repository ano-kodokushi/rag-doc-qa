...
6. **`multi_hop` 至今没有解，但它的病因已经拆开了**：试过按小节整理上下文（无净收益）、rerank（**对多跳零效果**）、
   **「扩窗口 + rerank」联合**（0 元机制诊断即证伪：**8 个席位下 rerank 照样会把多跳依据挤掉一篇**）。
   ...
   **① 池子里就没有 3/20 · ② 进了池子但节没进窗口 4/20 · ③ 小节在块不在 2/20 · ④ 全都在场 11/20**。
   ...
   所以**不要说"多跳问题已解决"**，也**不要说"是某一个原因"**（详见 `docs/EXPERIMENTS.md` E-02~E-07）。
